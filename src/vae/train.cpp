...
//
inline size_t get_conv_size(size_t width, size_t kernel_size, size_t padding_size, size_t stride){
    return (width - kernel_size + 2*padding_size)/stride + 1;
}

inline size_t get_deconv_size(size_t width, size_t kernel_size, size_t padding_size, size_t stride){
    return (width * stride) + padding_size - 1;
}

void construct_ae(tiny_dnn::network<tiny_dnn::sequential> &nn, size_t image_size){
    tiny_dnn::convolutional_layer cnv1(image_size, image_size, 5, 1, 6, tiny_dnn::padding::valid, true, 2, 2); //118
    tiny_dnn::relu_layer cnv1_relu();
    tiny_dnn::convolutional_layer cnv2(cnv1.out_shape()[0].width_, cnv1.out_shape()[0].height_, 5, cnv1.out_shape()[0].depth_, 12, tiny_dnn::padding::valid, true, 2, 2);//56
    tiny_dnn::relu_layer cnv2_relu();
    tiny_dnn::convolutional_layer cnv3(cnv2.out_shape()[0].width_, cnv2.out_shape()[0].height_, 5, cnv2.out_shape()[0].depth_, 12, tiny_dnn::padding::valid, true, 2, 2);//25
    tiny_dnn::relu_layer cnv3_relu();
    tiny_dnn::convolutional_layer cnv4(cnv3.out_shape()[0].width_, cnv3.out_shape()[0].height_, 5, cnv3.out_shape()[0].depth_, 12, tiny_dnn::padding::valid, true, 2, 2);//10
    tiny_dnn::relu_layer cnv4_relu();
    tiny_dnn::convolutional_layer cnv5(cnv4.out_shape()[0].width_, cnv4.out_shape()[0].height_, 5, cnv4.out_shape()[0].depth_, 12, tiny_dnn::padding::valid, true, 2, 2);//10
    tiny_dnn::relu_layer cnv5_relu();
    //fully connected layer
    tiny_dnn::fully_connected_layer fc1(cnv5.out_shape()[0].size(), 64);
    tiny_dnn::relu_layer fc1_tanh();
    tiny_dnn::deconvolutional_layer decnv1(2, 2, 7, 16, 16, tiny_dnn::padding::valid, true, 2, 2);//10
    tiny_dnn::relu_layer decnv1_relu();
    tiny_dnn::deconvolutional_layer decnv2(decnv1.out_shape()[0].width_, decnv1.out_shape()[0].height_, 7, decnv1.out_shape()[0].depth_, 16, tiny_dnn::padding::valid, true, 2, 2);//26
    tiny_dnn::relu_layer decnv2_relu();
    tiny_dnn::deconvolutional_layer decnv3(decnv2.out_shape()[0].width_, decnv2.out_shape()[0].height_, 5, decnv2.out_shape()[0].depth_, 16, tiny_dnn::padding::valid, true, 2, 2);//56
    tiny_dnn::relu_layer decnv3_relu();
    tiny_dnn::deconvolutional_layer decnv4(decnv3.out_shape()[0].width_, decnv3.out_shape()[0].height_, 5, decnv3.out_shape()[0].depth_, 16, tiny_dnn::padding::valid, true, 2, 2);//116
    tiny_dnn::relu_layer decnv4_relu();
    tiny_dnn::deconvolutional_layer decnv5(decnv4.out_shape()[0].width_, decnv4.out_shape()[0].height_, 5, decnv4.out_shape()[0].depth_, 16, tiny_dnn::padding::valid, true, 2, 2);//236
    tiny_dnn::relu_layer decnv5_relu();
    tiny_dnn::deconvolutional_layer decnv6(decnv5.out_shape()[0].width_, decnv5.out_shape()[0].height_, 5, decnv5.out_shape()[0].depth_, 16, tiny_dnn::padding::valid, true, 2, 2);//240
